...
export default function VoiceChatPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && 'speechSynthesis' in window) {
      ...
      if (speechSynthesisRef.current.getVoices().length === 0) {
        speechSynthesisRef.current.addEventListener('voiceschanged', loadVoices, { once: true })
      }
    }
  ...
  const startVoiceSession = async () => {
    try {
      ...
      console.log(`[Voice Chat Frontend] Starting session with ID: ${newSessionId}`)
      ...
      if (data.success) {
        ...
        setError(null)
        console.log(`[Voice Chat Frontend] Session started successfully: ${newSessionId}`)
      } else {
        console.error(`[Voice Chat Frontend] Session start failed:`, data)
        setError('Failed to start voice session')
      }
    } catch (error) {
      console.error('[Voice Chat Frontend] Error starting voice session:', error)
  ...
  const processAudioInput = async (audioBlob: Blob) => {
    ...
    console.log(`[Voice Chat Frontend] Processing audio for session: ${sessionId}`)
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        console.log(`[Voice Chat Frontend] Audio converted to base64, length: ${base64Audio.length}`)
        ...
        if (data.success) {
          ...
          setConversationTurns(prev => 
            prev.map((turn, index) => 
              index === prev.length - 1 && turn.type === 'user'
                ? { ...turn, status: 'complete', content: data.transcription || 'Voice message' }
                : turn
            )
          )
          ...
          if (data.shouldSpeak && data.response) {
            console.log(`[Voice Chat Frontend] Speaking response: ${data.response}`)
  ...
  const speakResponse = (text: string) => {
    ...
    console.log(`[Voice Chat Frontend] Speech synthesis supported: ${isSpeechSupported}`)
                ...
                >
                  <Box
                    sx={{
                      ...
                      height: 48,
                      borderRadius: '50%',
                      backgroundColor: turn.type === 'user' ? 'primary.main' : 'secondary.main',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      flexShrink: 0
                  ...
                  <Card 
                    ...
                    sx={{ 
                      minWidth: 200,
                      maxWidth: '70%',
                      backgroundColor: turn.type === 'user' ? 'primary.light' : 'grey.100'
                    }}
                  >
                    <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                      <Typography 
                        variant="body1" 
                        color={turn.type === 'user' ? 'primary.contrastText' : 'text.primary'}
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
                        ...
                        <Typography 
                          variant="caption" 
                          sx={{ 
                            color: turn.type === 'user' ? 'primary.contrastText' : 'text.secondary',
                            opacity: 0.7
                          }}
                        >
                          {turn.status === 'complete' ? turn.timestamp.toLocaleTimeString() : turn.status}
                        </Typography>
                      </Box>
                    </CardContent>
                  </Card>
